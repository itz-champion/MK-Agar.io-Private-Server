(function() {
 ...
 (function() {
	...
	const textColoration = (ctx, str, x, y, maxWidth, operation) => {
		...
		let sections = str.split("§").map(s => {
			let color = colors[parseInt(s.charAt(0), 16)];
			return {
				color: color,
				str: color ? s.slice(1) : (s ? "§" + s : s)
			};
		});
		if (begin) {
			sections.unshift({
	...
	proto.measureTextFormatted = function(str) {
		return this.measureText(str.replace(/§[\da-f]/gi, ""));
	};
})();
...
// ctx.textAlign = "right";
// ctx.fillTextFormatted(`§1hi, §2hello, §3world!`, x, y += 40);
// ctx.textAlign = "center";
// ctx.fillTextFormatted(`§4hi, §5hello, §6world!`, x, y += 40);
// ctx.textAlign = "left";
// ctx.fillTextFormatted(`§7hi, §8hello, §9world!`, x, y += 40);
//
// ctx.textAlign = "end";
// ctx.strokeTextFormatted(`§ahi, §bhello, §cworld!`, x, y += 40);
// ctx.textAlign = "center";
// ctx.strokeTextFormatted(`§dhi, §ehello, §fworld!`, x, y += 40);
// ctx.textAlign = "start";
// ctx.strokeTextFormatted(`hi, §hello, §world!`, x, y += 40);
// ctx.fillTextFormatted(`hi, hello, world!`, x, y += 40);
    ...
    request("https://mkskinsserver.glitch.me/skinList.txt", function(data) {
    ...
    function buildGallery() {
        ...
        for (var i = 0; i < sortedKeys.length; i++) {
            ...
            c += '<img class="circular" src="https://mkskinsserver.glitch.me/' + name + '.png">';
